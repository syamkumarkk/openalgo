...
def check_signals(marketOnTheDay=0):
    # -----------------------------
...
while True:
    ...
    try:
        if posflag==0:
                ...
                if len(runstatus)>0:
                    open_orders_status = [
                                o for o in runstatus if o.get("order_status") == "open" and o.get("strategy", "").startswith("5EMA_NIFTY")
                            ]
                    if len(open_orders_status)==2:
                        ...
                        client.cancelorder(order_id=open_orders_status[0]['orderid'], strategy="5EMA_NIFTY")
