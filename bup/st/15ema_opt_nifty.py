...
def check_signal(marketOnTheDay=1):
    ...
    if len(df) < 3:
        print(f"❌ Not enough candles for EMA(15). Candles available: {len(df)}")
        return None
    # -----------------------------
    # Calculate EMA(15)
    # -----------------------------
    ...
    print(
        ...
        f"EMA(15): {current_candle['ema_15']}"
    )
    ...
    previous_ema = completed["ema_15"]
    previous_low  = completed["low"]
    previous_high  = completed["high"]
    current_ema = current["ema_15"]
    ...
    print(
        ...
        f"Low: {previous_low} | EMA(15): {previous_ema}"
    )
    print(
        f"Time: {df.index[-1].strftime('%H:%M')} | "
        f"Low: {current_low} | High: {current_high} | EMA(15): {current_ema}"  
    )


    return main_obj.order_util.identify_15ema_trigger(previous_ema,previous_low,previous_high,current_low,current_high)

runstatus =  main_obj.order_util.get_orders_by_stratagy("15EMA")
running_orders=[]
# posflag =  main_obj.order_util.get_post_flag(runstatus,running_orders,"15EMA")
posflag =  len(runstatus)
                    ...
                    if (runstatus!=None):
                        ...
                        if runstatus.get("BUY"):
                            ...
                            if posflag==1:
                                ...
                                # Target order
                                order_utilObj.trigger_15ema_bracketOrder(symbol,price,slprice,orderid)                    
            else:   
                    print("✅ Trail running order that is active or running to clear missing orders")        
                    runstatus =  main_obj.order_util.get_orders_by_stratagy("15EMA")
                    running_orders=[]
                    if len(runstatus)>0:

                        completed_buy_orders_status = [
                                    o for o in runstatus if o.get("order_status") == "complete" and o.get("action") == "BUY" and o.get("strategy", "").startswith(f"15EMA_{main_obj.index}")
                                ]
                        for opened_buy in completed_buy_orders_status:                            
                            completed_sell_orders_status = [
                                        o for o in runstatus if o.get("action") == "SELL" and o.get("strategy", "").startswith(f"15EMA_{main_obj.index}_{opened_buy['orderid']}")
                                    ]
                            if len(completed_sell_orders_status)==0:
                                print(f"🚀 COMPLETED SELL ORDERS IN 15EMA stratagy for orderid {opened_buy['orderid']}:\n")
                                symbol = opened_buy['symbol']
                                ...
                                print(f'trigger_price : {trigger_price}')
                                # Target order
                                order_utilObj.trigger_15ema_bracketOrder(symbol,price,low_price,opened_buy['orderid'])                    

                        open_orders_status = [
                                    o for o in runstatus if o.get("order_status") == "open" and o.get("strategy", "").startswith(f"15EMA_{main_obj.index}")
                                ]
                        ...
                        time.sleep(1)
                        manage_trades_untracked("15EMA",sl_order,target_order["orderid"],parent_order_id,atm_ltp)
                        time.sleep(1)
                    open_orders_status = [
                                    o for o in runstatus if o.get("order_status") == "open" and o.get("strategy", "").startswith(f"15EMA_{main_obj.index}")
                                ]
                    todays_orders_status = [
                                    o for o in runstatus if o.get("order_status") == "BUY" and o.get("strategy", "").startswith(f"15EMA_{main_obj.index}")
                                ]
