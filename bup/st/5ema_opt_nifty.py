        ...
        try:
            ...
            if posflag==0:
                    ...
                    if len(runstatus)>0:
                            ...
                            if atm_ltp is None:
                                ...
                                if atm_ltp>0:
                                    ...
                                    if(atm_ltp>=new_target_price_escalation and sl_order["trigger_price"]<new_target_price_escalation and sl_order["trigger_price"]!=new_sl_price and sl_order["trigger_price"]<new_sl_price):
                                        ...
                                        parent_order_id = target_order["strategy"].removeprefix(prefix).split("_")[1]  
                                        strategy_buyed = [
                                            o for o in runstatus if o.get("order_status") == "open" and o.get("strategy", "").startswith(f"5EMA_{self.parent.index}_{parent_order_id}_BUY")
                                        ]
                        ...
                        time.sleep(1)
                        order_utilObj.manage_trades_untracked("5EMA",sl_order,target_order["orderid"],parent_order_id,atm_ltp)
                        time.sleep(1)
                    open_orders_status = [
                                    o for o in runstatus if o.get("order_status") == "open" and o.get("strategy", "").startswith(f"5EMA_{main_obj.index}")
                                ]
